    ...
    <section id="projects" className="py-24 lg:py-32" ref={ref}>
      <div className="container mx-auto px-4">
        <div className="mx-auto max-w-2xl text-center">
          <span className="font-heading text-sm uppercase tracking-widest text-gold">Portfolio</span>
        ...
        <div className="mt-10 flex flex-wrap justify-center gap-3">
          {filters.map((f) => (
            <button
              key={f}
              onClick={() => setActive(f)}
              className={`rounded-sm px-5 py-2 font-heading text-sm uppercase tracking-wider transition-all duration-300 ${
                active === f
        ...
        <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((project, i) => (
            <div
              key={project.title}
              className={`group relative overflow-hidden rounded ${isInView ? "animate-scale-in" : "opacity-0"}`}
            ...
            >
              <div className="relative aspect-[4/3] overflow-hidden">
                <img
                  ...
                  loading="lazy"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-charcoal-deep via-charcoal-deep/30 to-transparent opacity-60 transition-opacity duration-300 group-hover:opacity-90" />
                <div className="absolute inset-0 flex flex-col justify-end p-6 translate-y-4 opacity-0 transition-all duration-500 group-hover:translate-y-0 group-hover:opacity-100">
                  <span className="font-heading text-xs uppercase tracking-widest text-gold">{project.category} · {project.year}</span>
